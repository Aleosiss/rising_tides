...
function RTGameState_MeldEffect Initialize(XComGameState_Unit MeldMaker)
{
	...
	`RTLOG("Initializing new MeldEffect GameState.");
	...
	{
		`RTLOG("No parent Meld found, setting this unit as the host.");
	...
	{
		`RTLOG("Meld Parent GameState found:");
   	...
   	`RTLOG("" @ XComGameState_Unit(`XCOMHISTORY.GetGameStateForObjectID(GameStateHost.ObjectID)).GetName(eNameType_Full) @" has finished initalizing its RTGameState_MeldEffect.");
	return self;
}
...
simulated function EventListenerReturn AddUnitToMeld(Object EventData, Object EventSource, XComGameState GameState, Name EventID, Object CallbackData)
{
	...
	`RTLOG("AddUnitToMeld called by " @ EventID);
	...
	//`RTLOG("" @ GameStateHostUnit.GetName(eNameType_Full) @ "'s meld has " @ UpdatedMeldEffect.Members.Length @ " people in it.");
...
simulated function EventListenerReturn RemoveUnitFromMeld(Object EventData, Object EventSource, XComGameState GameState, Name EventID, Object CallbackData)
{
	...
	`RTLOG("RemoveUnitToMeld called by " @ EventID);
	History = `XCOMHISTORY;
