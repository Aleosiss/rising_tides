...
class RTEffect_TimeStopEnd extends X2Effect;

var name EffectToRemove;
...
simulated protected function OnEffectAdded(const out EffectAppliedData ApplyEffectParameters, XComGameState_BaseObject kNewTargetState, XComGameState NewGameState, XComGameState_Effect NewEffectState)
{
	...
	if (OldTargetUnitState != none)
	{
		...
		foreach OldTargetUnitState.AffectedByEffects(EffectRef)
		{
			...
			if (Effect.GetX2Effect().EffectName == EffectToRemove && !Effect.bRemoved)
			{
		...
		`LOG("Rising Tides: " @ OldTargetUnitState.GetFullName() @ " has rejoined time.");
	}
	else
	{
		`LOG("Rising Tides: Time Stop End attempted failed (no primary target).");
	}
