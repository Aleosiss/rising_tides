...
function RegisterForEvents(XComGameState_Effect EffectGameState)
{
	...
	local RTGameState_Effect RTEffectGameState;

	EventMgr = `XEVENTMGR;

	RTEffectGameState = RTGameState_Effect(EffectGameState);

	EffectObj = RTEffectGameState;

	// Register for the required events
	// At the start of every turn, each Aura Source checks what Units it should affect
	EventMgr.RegisterForEvent(EffectObj, 'PlayerTurnBegun', RTEffectGameState.OnTotalAuraCheck, ELD_OnStateSubmitted);
}
