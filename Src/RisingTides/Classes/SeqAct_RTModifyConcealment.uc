...
//-----------------------------------------------------------
class SeqAct_RTModifyConcealment extends SequenceAction;
...
event Activated()
{
	...
	else if( Unit != None )
	{
		...
		{
			if( Unit.IsConcealed() && !Unit.HasSoldierAbility('RTGhostInTheShell') )
			{
		...
		else if( InputLinks[2].bHasImpulse )
		{
			if( Unit.IsConcealed() )
			{
				if(!Unit.HasSoldierAbility('RTGhostInTheShell') ) {
					Unit.BreakConcealment();
				}
			}
			else
...
defaultproperties
{
	...
	ObjName="RT Modify Unit or Squad Concealment"
	bCallHandler = false
