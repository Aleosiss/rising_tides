...
function array<SoldierClassAbilityType> GetEarnedSoldierAbilities()
{
	...
	local int i;
	local X2EarnedAbilityContainer AbilityContainer; // LW  added
	...
	//LW set up a container for the Earned Abilities array
	AbilityContainer = new class'X2EarnedAbilityContainer';
	AbilityContainer.EarnedAbilities = EarnedAbilities;

	//LW set up a Tuple -- holds a container with the entire list of EarnedAbilities for modification
	OverrideTuple = new class'XComLWTuple';
	OverrideTuple.Id = 'ModifyEarnedSoldierAbilities';
	OverrideTuple.Data.Add(1);
	OverrideTuple.Data[0].kind = XComLWTVObject;
	OverrideTuple.Data[0].o = AbilityContainer;

	//LW add hook for mods to modify earned soldier abilities (generally adding additional sources)
	`XEVENTMGR.TriggerEvent('ModifyEarnedSoldierAbilities', OverrideTuple, self);
	EarnedAbilities = X2EarnedAbilityContainer(OverrideTuple.Data[0].o).EarnedAbilities;
