...
{
    // LWS: If the yellow alert system is enabled consider throttling beacons to be the
    // lowest priority.
    if (class'Helpers_LW'.static.YellowAlertEnabled())
    {
        if (m_arrAlertData[iAlertA].AlertCause == eAC_ThrottlingBeacon)
        {
            return false;
        }
        else if (m_arrAlertData[iAlertB].AlertCause == eAC_ThrottlingBeacon)
        {
            return true;
        }
    }
