...
simulated function QueueScamperBehavior(XComGameState_Unit ScamperUnitState, XComGameState_Unit AlertSourceState, bool bSurprisedScamper, bool bFirstScamper)
{
	...
	if (ScamperUnitState.CanScamper())
	{
		...
		{
            // LWS: Removed. Units can (rarely) spawn on top of fires in certain maps. This causes them to
            ...
            // a pending scamper. Just have them activate immediately (they'll activate as soon as squad
            // concealment breaks anyway) so the movement keeps chugging along.
